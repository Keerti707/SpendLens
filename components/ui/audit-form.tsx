...
const defaultForm: AuditFormState = {
    tools: [
        {
            id: "tool-1",
            tool: "",
...
export function AuditForm() {
    const [form, setForm] = useState<AuditFormState>(defaultForm);
    ...
    function updateRootField(field: "teamSize" | "useCase", value: string) {
        setForm((currentForm) => ({
            ...currentForm,
            [field]: value,
        }));
        ...
        setForm((currentForm) => ({
            ...
            tools:
                currentForm.tools.length === 1
                    ? currentForm.tools
                    : currentForm.tools.filter((tool) => tool.id !== toolId),
        }));
    ...
    function handleGenerateAudit() {
        const hasEmptyTool = form.tools.some(
            (tool) => !tool.tool || !tool.plan || !tool.monthlySpend || !tool.seats
        );

        if (hasEmptyTool || !form.teamSize || !form.useCase) {
            setError("Please complete every tool row before generating an audit.");
            setAuditResult(null);
            return;
        }

        const hasInvalidPlan = form.tools.some((tool) => {
            const validPlans = toolPlans[tool.tool as keyof typeof toolPlans];
            return !validPlans.includes(tool.plan);
        });

        if (hasInvalidPlan) {
            setError("One selected plan does not exist for its AI tool.");
            setAuditResult(null);
            return;
        }
    ...
    return (
        <Card className="mt-20 w-full max-w-5xl border border-white/10 bg-white/[0.04] p-8 text-left shadow-2xl shadow-indigo-500/10 backdrop-blur-xl">
            <div className="mb-8 flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
                <div>
                    ...
                    <p className="mt-2 text-sm text-white/55">
                        Add every AI tool your team pays for. SpendLens checks plan fit,
                        overlap, and savings opportunities.
                    </p>
                </div>
            ...
            <div className="space-y-5">
                {form.tools.map((toolEntry, index) => {
                    const availablePlans = toolEntry.tool
                        ? toolPlans[toolEntry.tool as keyof typeof toolPlans]
                        : [];
                    ...
                    return (
                        ...
                        >
                            <div className="mb-4 flex items-center justify-between">
                                ...
                                <button
                                    onClick={() => removeTool(toolEntry.id)}
                            ...
                            <div className="grid gap-5 md:grid-cols-4">
                                <div className="space-y-2">
                                    ...
                                    <Select
                                        ...
                                        onValueChange={(value) =>
                                            updateTool(toolEntry.id, "tool", value)
                                        }
                                    >
                                ...
                                <div className="space-y-2">
                                    ...
                                    <Select
                                        ...
                                        onValueChange={(value) =>
                                            updateTool(toolEntry.id, "plan", value)
                                        }
                                ...
                                <div className="space-y-2">
                                    <Label className="text-white/80">Spend ($/mo)</Label>
                                ...
                                <div className="space-y-2">
                                    ...
                                    <Input
                                        ...
                                        onChange={(event) =>
                                            updateTool(toolEntry.id, "seats", event.target.value)
                                        }
                                    />
                                </div>
                            </div>
            ...
            <div className="mt-10 border-t border-white/10 pt-8">
                <div className="mb-6">
                    <h3 className="text-lg font-semibold text-white">Team Context</h3>

                    <p className="mt-1 text-sm text-white/50">
                        These settings apply across your entire AI stack audit.
                    </p>
                </div>

                <div className="grid gap-6 md:grid-cols-2">
                    <div className="space-y-2">
                        <Label className="text-white/80">Total Team Size</Label>
                        ...
                        <Input
                            ...
                            value={form.teamSize}
                            onChange={(event) =>
                                updateRootField("teamSize", event.target.value)
                            }
                        />
                    </div>
                    ...
                    <div className="space-y-2">
                        ...
                        <Select
                            value={form.useCase}
                            onValueChange={(value) => updateRootField("useCase", value)}
                        >
                            <SelectTrigger className="h-12 w-full border-white/10 bg-black/40 text-white">
            ...
            {auditResult && (
                <AuditReport auditResult={auditResult} auditRunCount={auditRunCount} />
            )}
        </Card>
    );
}